...
class Dataset(object):
    def __init__(self, conf):
        ...
        self.item_popularity = self._count_item_popolarity()
        self.user_activity = self._count_user_activity()
    ...
    def _count_item_popolarity(self):
        colsum = np.array(self.train_matrix.sum(0))
        return np.squeeze(colsum)

    def _count_user_activity(self):
        rowsum = np.array(self.train_matrix.sum(1))
